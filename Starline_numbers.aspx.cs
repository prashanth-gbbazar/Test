...
public partial class Starline_numbers : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            Glib.LoadRequest("select * from [starline_numbers] order by id desc", gvsprovider);
            gameBind();
        }
    ...
    public void clear()
    {
        tdigit.Text = tdate.Text = topen.Text = "";
        btnAdd.Text = "Submit";
        gameBind();
    }
    ...
    protected void gvsprovider_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName.Equals("Del"))
            {
                ...
                lblid.Text = id;
                Alib.idExecute("delete from starline_numbers where id=" + id);
                Glib.LoadRequest("select * from starline_numbers order by id desc", gvsprovider);
            ...
            if (e.CommandName.Equals("edi"))
            {
                try
                {
                    ...
                    cmd = new SqlCommand("select * from starline_numbers where id='" + lblid.Text + "'", conn);
                    conn.Open();
        ...
        try
        {
            ...
            else
            {
                if (btnAdd.Text != "Update")
                {
                    ...
                    SqlCommand cmd;
                    con.Open();
                    string sql = "insert into starline_numbers (opennumber, digit, gameid, game, date, time) values('" + topen.Text + "', '" + tdigit.Text + "', '" + dgame.SelectedValue + "', '" + dgame.SelectedItem.Text + "', '" + tdate.Text + "', '" + dopen1.SelectedItem.Text + " : " + dopen2.SelectedItem.Text + " " + dopen3.SelectedItem.Text + "')";
                ...
                {
                    Alib.idExecute("update starline_numbers set opennumber='" + topen.Text + "',digit='" + tdigit.Text + "', date = '" + tdate.Text + "' where id='" + lblid.Text + "'"); 
                ...
                Glib.LoadRequest("select * from starline_numbers order by id desc", gvsprovider);
                sendList1(topen.Text, tdigit.Text, dgame.SelectedItem.Text, tdate.Text);
                clear();
                return;
            }
        }
    ...
    public void sendList1(string open, string digit, string game, string date)
    {
        try
        {
            ...
            {
                con.Open();
                using (SqlCommand cmd = new SqlCommand("select DISTINCT nkey from notify where nkey <> '' and isstarline=1", con))
                {
            ...
            sendNotificationFCM(list1, open, digit, game, date);
        }
        catch (Exception ex)
        {
    ...
    public void sendNotificationFCM(List<string> gcmid, string open, string digit, string game, string date)
    {
        ...
        string bodymsg = topen.Text + " - " + tdigit.Text;
